...
# fenetre a gauche
        ...
        # label_level_jeu
        self.label_level = QLabel("Level 4", self.win_gauche)
...
# plateau du jeu a droite
    ...
    def try_move(self, i,j):  
        ...
        if abs(empty_i - i) + abs(empty_j - j)== 1:
            ...
            self.clack.play()
            self.update_board()
            self.label_img_original.setPixmap(QPixmap(self.img_path).scaled(0,0))
            self.step += 1
            self.nbr_pas.setText(f"{self.step}")
            ...
            if self.is_solved():
                ...
                self.parent.aller_niveau_suivant("Débutant")
